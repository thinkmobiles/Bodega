package com.thinkmobiles.bodega;

/**
 * Created by denis on 21.10.15.
 */
public class Constants {

    public static final String YOUTUBE_API_KEY = "AIzaSyDsvbQ4n4y4t2P3-CnO3QvmhU_ut5gmFMQ";
    public static final String YOUTUBE_PACKAGE_NAME = "com.google.android.youtube";

    // SHARED PREFS CONSTANTS
    public static final String SHARED_PREFS_NAME = "SHARED_PREFS_NAME";
    public static final String SHARED_PREFS_LAST_UPDATE_KEY = "SHARED_PREFS_LAST_UPDATE_KEY";

    public static final int LEVEL_FIRST = 1;
    public static final int LEVEL_SECOND = 2;
    public static final int LEVEL_THIRD = 3;

    // BUNDLE KEYS
    public static final String ALL_LEVELS_MODEL_ARG = "ALL_LEVELS_MODEL_ARG";
    public static final String DOWNLOADED_FILE_ARG = "downloaded_file_arg";

    // IDS
    public static final String TANQUES_ID = "479";
    public static final String LOGOTIPOS_ID = "502";
    public static final String COLUMNAS_ID = "475";
    public static final String ARTICULOS_DE_USO_ID = "499";
    public static final String COMPANIA_ID = "467";
    public static final String CARACTERISTICS_ID = "470";
    public static final String TIRAGE_ID = "471";
    public static final String LOGISTICA_ID = "472";
    public static final String EJEMPLOS_ID = "477";
    public static final String GALERIA_DE_ACERO_ID = "503";
    public static final String GALERIA_DE_COBRE_ID = "504";
    public static final String FICHA_TACNICA_DE_COLUMNAS_ID = "506";
    public static final String FICHA_TACNICA_DE_BANDEJAS_ID = "505";
    public static final String TOLDOS_ID = "507";
    public static final String VINILIS_ID = "508";
    public static final String GRAFICAS_ID = "509";
    public static final String APLICACIONES_ID = "501";
    public static final String CON_VOLUMEN_ID = "481";
    public static final String LONA_ID = "482";
    public static final String SPRAY_ID = "483";
    public static final String TEXTIL_ID = "485";
    public static final String TELA_DE_SACO_ID = "484";
    public static final String DIBOND_ID = "491";
    public static final String AZULEJO_ID = "492";
    public static final String MESA_VUELTA_ID = "493";
    public static final String COLONIAL_ID = "494";
    public static final String LEYENDA1_ID = "495";
    public static final String CAJA_DE_CERVEZAS_ID = "496";
    public static final String PALET_ID = "497";
    public static final String CHOPO_ID = "498";
    public static final String TELA_DE_SACO1_ID = "488";
    public static final String HAMACA_ID = "489";
    public static final String LONA_MICROPERFORADA_ID = "490";
    public static final String PAPEL_PINTADO_ID = "510";
    public static final String LEYENDA_ID = "511";
    public static final String ELEMENTOS_DECORATIVOS_ID = "487";
    public static final String ENFIRADORES_ID = "478";
    public static final String COMPRESSOR_ID = "476";

    public static final String INICIO_ID = "inicio";
    public static final String ENVIOS_ID = "envios";

    // EXTRAS
    public static final String EXTRA_MODEL = "extra_model";
    public static final String EXTRA_ID = "extra_id";
    public static final String EXTRA_ITEM = "extra_item";
    public static final String EXTRA_VIDEO = "extra_video";
    public static final String EXTRA_PRODUCT = "extra_product";
    public static final String EXTRA_FLAG_1 = "extra_flag_1";
    public static final String EXTRA_FLAG_2 = "extra_flag_2";
    public static final String EXTRA_FLAG_3 = "extra_flag_3";

    
    // Splash Activity broadcast actions
    public static final String ACTION_DOWNLOAD_SUCCESS = "action_download_success";
    public static final String ACTION_DOWNLOAD_FAILED = "action_download_failed";
    public static final String ACTION_FILE_DOWNLOADED = "action_file_downloaded";

}
